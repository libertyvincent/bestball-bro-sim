...
**Availability (draw-zeroing) fields.** Each player record carries `availability_p_miss` (per-player missed-week probability, `max(0, 1 - expected_games[pos]/clay_games_i)`), and `_meta` carries an `availability_mechanism` marker (`type: "draw_zeroing"`, `missed_week_model: "iid_bernoulli"`, `expected_games`, canonical 17-game `p_miss`) — this **replaces** the old `_meta.availability_adjustment` scaling summary (the key was renamed `availability_adjustment` → `availability_mechanism`). `season_mean`/`season_std`/`season_percentiles`, VOR/tier/`position_rank`, and per-week `mean`/`std`/`percentiles` are all recomputed post-sim from the masked draws. Per-week `mean`/`std` are **unconditional** (they include the miss chance), so `Σ_active weekly.mean ≈ season_mean` still holds. At attrition-heavy positions a weekly `p10` can legitimately be 0 (RB: `p_miss ≈ 0.106 > 0.10`).

Two consequences of recomputing from draws, both ratified (do not re-flag):

- **`season_mean` is now the empirical, clip-aware estimator** (mean of the masked draws), not PR #34's analytical level. The per-week `pmax(0, ·)` clip the analytical mean omitted lifts the level slightly, scaling with position CV: ~+0.0% QB / +0.5% RB / +0.8% WR / +1.2% TE. So the per-position gap vs the Underdog market reads **WR ≈ +0.5%, TE ≈ −1.8%** (not PR #34's WR −0.3% / TE −3.0%). This is the more-correct (no-negative-scores) level, within the ±2% mean-preservation budget.
- **`season_std` rises** — it now includes the mask variance and is the empirical post-mask std. The rise is largest for high-μ studs (the `(1−q)·μ²` term dominates their otherwise-tight variance): **stud RB ≈ +25%** vs the deployed PR #34 std, decomposing as `rebase(1/q) × mask × clip-trim` (PR #34's σ was computed at the *scaled* μ_w and understated played-week spread). Disagreement-dependent; smaller at QB/WR/TE. The old `season_std² = disagreement_std² + aleatoric_std²` invariant is **retired** (`disagreement_std`/`aleatoric_std` remain as the pre-mask conditional components).

See DRAW_ZEROING_DESIGN.md.
...
**Availability (draw-zeroing) lives outside the parquet.** `draw_value` is the *conditional-on-playing* weekly score (0 only on byes). Position-level injury attrition is priced as a separate per-(path, player, week) missed-week mask — kept w.p. `q_i = min(1, expected_games[pos]/clay_games_i)` — that is realized **independently** in all four scoring consumers, each reading the same per-player `availability_p_miss`: (a) the projections season-stat recompute (`R/simulate.R`), (b) the Artifact A tensor build (`R/ev_blocks.R`), (c) the shipped tournament-curve field build (`R/tournament_ev.R` via `deploy_ev_blocks.R`), and (d) the BBMDB/xAdv validator (`R/bbmdb_validator.R`). All share `R/correlation.R::.mask_matrix_list`, applied **post-inverse-CDF**. It is deliberately **not** written into the parquet: doing so would entangle the mask with the copula's inverse-CDF map. The copula/lineup/marginal primitives are mask-exempt by contract (the mask is applied to their inputs/outputs by callers). The per-player rate is published as `availability_p_miss` in the projection record, and the mechanism is described in `_meta.availability_mechanism`. See DRAW_ZEROING_DESIGN.md.
