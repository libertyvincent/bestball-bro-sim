...
- **Old (raw, local-dev only) — the `inst/data` scraper export:** `inst/data/scraped_drafts/udbb-scraper-*.json`. These are **un-stripped** (raw `user_id` UUIDs + `/v1/user` envelopes) and are **git-ignored** (`.gitignore:51`) — never committed, never in history. Read by `load_scraped_drafts()` as a **local-dev** input; in production `deploy_ev_blocks.R::.resolve_field_targets` falls back to the committed digest (CI) or ADP defaults, so nothing shipped depends on the raw file. _(Retirement of this old local lineage in favor of the stripped corpus is a pending hub decision — see the Task A audit.)_
